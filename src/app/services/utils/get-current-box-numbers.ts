...
export function getCurrentBoxNumbers(
  ...
  var rowOne =
    Math.floor(boxNumber / gridProps.gridBoxesCols) * gridProps.boxCols;
  ...
  for (var i = 0; i < gridProps.boxCols; i++) {
    ...
    boxColumnNumbers.push(
      ...filterBoxList(currentCol, rowOne, gridProps.boxRows)
    );
  }
  ...
  for (var i = 0; i < gridProps.boxRows; i++) {
    ...
    boxRowNumbers.push(
      ...filterBoxList(currentRow, columnOne, gridProps.boxRows)
    );
  }
